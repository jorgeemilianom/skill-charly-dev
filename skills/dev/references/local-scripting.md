...
`scripts/local/` is a growing, per-project toolbox of small scripts this skill family writes for
itself. It's gitignored — unlike `scripts/jira-communication/` (shared, tracked upstream), these are
...
1. Write it to `scripts/local/<descriptive-name>.<sh|py>`.
...
4. `chmod +x` it.
5. Register it in `scripts/local/MANIFEST.json`:
   ```json
   {
     "scripts": [
       {
         ...
         "usage": "scripts/local/<descriptive-name>.sh <args>",
         "created_at": "<ISO date>"
       }
     ]
   }
...
6. Mention it briefly to the user (one line — this is local, reversible, no external side effect, so it
   ...
   `scripts/local/` para no repetirlo la próxima vez."
